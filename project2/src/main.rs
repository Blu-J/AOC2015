...
fn line_check(input: &str) -> u32 {
    let list_numbers = input.split_whitespace().map(|x| x.parse::<u32>().unwrap());
    let min_max = list_numbers.fold(
        MinMaxA {
            min: u32::max_value(),
            max: u32::min_value(),
        },
...
fn check_sum(input: &str) -> u32 {
    input.lines().map(line_check).fold(0, |x, y| x + y)
}
fn main() {
    println!(
        "We have the answer of {}",
        check_sum(
...
mod tests {
    ...
    #[test]
    fn line1() {
        assert_eq!(line_check("5 1 9 5"), 8);
    }
    #[test]
    fn line2() {
        assert_eq!(line_check("7 5 3"), 4);
    }
    #[test]
    fn line3() {
        assert_eq!(line_check("1234"), 0);
    }
    #[test]
    fn line4() {
        assert_eq!(line_check("2 4 6 8"), 6);
    }
}
